...
## Normalize images
#x_scaled_test =  np.pi*(normalize(x_test_pca, norm='max', axis=1))
#x_scaled_train = np.pi*(normalize(x_train_pca, norm='max', axis=1))
...
def model(theta, x, num_qubits, reps=3):
    ...
    var_circ = ansatz(ansatz_type, num_qubits, reps)
...
def callback(theta):
    ...
    #if batch_index % 10 == 0:
    #    plt.figure()
    #    a1=[test_predictions[i] for i in range(len(y_test)) if y_test[i]==1]
    #    plt.hist(a1, bins=5)     
    #    a0=[test_predictions[i] for i in range(len(y_test)) if y_test[i]==0]
    #    plt.hist(a0, bins=5)
    #    plt.title("Histogram of predictions")
#
    #    plt.figure()
    #    plt.hist([(1.0-test_predictions[i])**2 for i in range(len(y_test)) if y_test[i]==1], bins=5)           
    #    plt.hist([(test_predictions[i])**2 for i in range(len(y_test)) if y_test[i]==0], bins=5)
    #    plt.title("Histogram of MSE")
#
    #    plt.figure()
    #    plt.hist([x_scaled_train[i][0] for i in range(len(y_test)) if y_train[i]==1], bins=5)
    #    plt.hist([x_scaled_train[i][0] for i in range(len(y_test)) if y_train[i]==0], bins=5)
    #    plt.title("Histogram of first elements of x_train")
#
    #    plt.figure()
    #    plt.hist([x_scaled_test[i][0] for i in range(len(y_test)) if y_test[i]==1], bins=5)
    #    plt.hist([x_scaled_test[i][0] for i in range(len(y_test)) if y_test[i]==0], bins=5)
    #    plt.title("Histogram of first elements of x_test")
#
    #    plt.figure()
    #    plt.hist(theta, bins=5)
...
#
    ...
    #Increase batch_size if batch_index is a multiple of 100
    #if batch_index>1 and batch_index % 100 == 0:
    #    global batch_size
    #    batch_size = batch_size + 50
    #    if batch_size > len(x_scaled_train):
    #        batch_size = len(x_scaled_train)
...
#for i in range(3):
#    res= minimizeSPSA(loss_function, x0=weights, a=1.0/(2**i), c=1.0/(2**i), niter=100, callback=callback, paired=False)
#    weights = res.x
